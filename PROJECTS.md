...
- **Backend:** реалтайм на WebSocket (Phoenix Channels), stateful-сервисы и отказоустойчивость на OTP/GenServer, REST API, Telegram Bot API, контейнеризация (Docker)
...
<img src="https://go-skill-icons.vercel.app/api/icons?i=elixir,phoenix,docker,railway" />
...
Реалтайм-бэкенд на Phoenix (Elixir), синхронизирующий состояние таймера между всеми участниками комнаты в реальном времени.

- Синхронизация по WebSocket через Phoenix Channels — состояние таймера едино для всех клиентов
- Каждая комната — отдельный процесс OTP (GenServer) под DynamicSupervisor + Registry
- Отслеживание участников онлайн через Phoenix Presence
- Отказоустойчивость: автоматический перевыбор хоста при отключении с grace-периодом, авто-очистка пустых комнат
- Готовность к кластеризации (dns_cluster), покрытие каналов тестами
...
- Реалтайм-синхронизация таймера между участниками комнаты по WebSocket
- Отказоустойчивый stateful-бэкенд на OTP: изолированный процесс на комнату, перевыбор хоста, авто-очистка
